...
また、**このツールは正式な検証を経ていません。** 不具合等が生じた場合は適宜修正するか、[アップロードするデータ](https://github.com/mkaba-gxd/aws/blob/v3.0.0/README.md#転送されるデータ)または[復帰させるデータ](https://github.com/mkaba-gxd/aws/blob/v3.0.0/README.md#転送されるデータ-1)を参照して該当データをawsコマンドでアップロード/ダウンロードしてください。\
計算ノードはawsコマンドがインストールされていないため、awsアップロード/ダウンロード用bashスクリプトはqmasterで実行してください。(長時間かかるので、nohupでバックグラウンド実行を推奨)
